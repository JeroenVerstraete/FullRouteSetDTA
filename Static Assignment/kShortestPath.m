function [shortestPaths] = kShortestPath(netCostMatrix, source, destination, k_paths)
...
if source > size(netCostMatrix,1) || destination > size(netCostMatrix,1)
    ...
    %---------------------INITIALIZATION---------------------
    k=1;
    [~,distance,path] = dijkstra(netCostMatrix, source, destination);
    cost = distance(destination);
    %options.end_points = destination;
    %[path cost] = perform_dijkstra(netCostMatrix,source,options);
    ...
    if isempty(path)
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                ...
                [~,distance,dev_p] = dijkstra(temp_netCostMatrix, P_(index_dev_vertex), destination);
