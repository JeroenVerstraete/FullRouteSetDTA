function [TurnFlows] = NodeModel_demand_prop(nbIncomingLinks,nbOutgoingLinks,sendingFlow,turningFractions,receivingFlow,incomingLinks_capacity)
%#codegen
%coder.inline('never');
...
distrFactors = turningFractions.*repmat(sendingFlow,1,nbOutgoingLinks); %demand proportional
...
while any(activeOutLinks)
    ...
    if any(sendingFlow(competingLinks(:,j))<=alpha_min*sendingFlow(competingLinks(:,j)))
        for a=1:nbIncomingLinks
            if competingLinks(a,j)
                if sendingFlow(a)<=alpha_min*sendingFlow(a)
